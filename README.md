...
- `libs/api_client/api_requests.py:`
    - `response_postcards:` извлекает и валидируют данные об открытках через Pydantic-модель, проверяя их формат на соответствие ожидаемым моделям.  
    - `response_mentors:` извлекает и валидируют данные о менторах через Pydantic-модель, проверяя их формат на соответствие ожидаемым моделям. 
...
- `libs/utils.py:` 
    ...
    - `long_message:` Обрезает сообщения, которые более 4096 длинной.
...
* `http://127.0.0.1:8002/mentor_info`
    ```python
        {"mentors": 
        [
            {"id": 1, "name": {"first": "Евгений", "second": "Devman"}, "tg_username": "@e", "tg_chat_id": 2321, "bday": "1991-02-23"}, 
            {"id": 2, "name": {"first": "Ильмир", "second": "Devman"}, "tg_username": "@il", "tg_chat_id": 32, "bday": None}, 
            {"id": 3, "name": {"first": "Роман", "second": "Грч"}, "tg_username": "@R", "tg_chat_id": 123123, "bday": "1991-02-23"}
        ]
        }
    ```
...
* `http://127.0.0.1:8002/name_template`
    ```python
        {"postcards": 
        [
            {"id": 1, "holidayId": "01.01", "name_ru": "Новый год", "body": "🎉✨#name, С Новым годом! Пусть этот год принесёт тебе радость, процветание и успех!✨🎉"}, 
            {"id": 2, "holidayId": "01.07", "name_ru": "Рождество Христово (Православие)", "body": "🌟 #name,С Рождеством Христовым! Христос рождается!🌟"}, 
            {"id": 40, "holidayId": "birthday", "name_ru": "День рождения", "body": "💐🌸 #name, С Днём рождения! Пусть твоя жизнь будет похожа на прекрасный сад, где каждое мгновение расцветает новыми красками! Желаю безграничного счастья, мира и благополучия! 🌸💐"}
        ]
        }
    ```
## Запуск
